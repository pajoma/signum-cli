...
- AC-20.7: The **query key** is validated against cached metadata before the request, so a typo costs no round trip, and against the same definition `signum queries` lists from. **Every token the caller names** — `--column`, `--order`, `--filter` and `--filter-json`, the last two at every nesting depth including a group's own token — is validated via `api/query/parseTokens` before the request, in **one round trip for all four slots**, with the server's own nearest matches or list of valid continuations in the error (#95, #97). Validation is a **superset** of what a filter accepts: `parseTokens` resolves with `SubTokensOptions.All` while a filter is parsed with `CanElement | CanAnyAll` plus conditional aggregate/time-series (`FilterJsonConverter.cs:87`), so a token that passes and is still illegal in a filter (`.Nested`, `.ToArray`, snippet, operation, manual) is refused locally with a message naming why, rather than arriving as the generic 500. *(Amended twice: an earlier version deferred column/order validation to `m2` on the REQ-012 dependency — that dependency shipped and the deferral outlived it, with validation happening only as an accident of the `--resolve` flag. A second version, after #95, recorded filter tokens as the remaining piece; #97 closed it.)*
...
**Acceptance Criteria:**
- AC-24.1: Commands list the app's queries, types, and each type's members, entity kind, and operations, from the cached `api/reflection/types`. Reflection members are labelled as **the reflection view of the entity, not the query-token namespace**, because they are not interchangeable: a mixin field is reported as `[SomeMixin].Field` and is queryable as `Field`. `explain <Type>` therefore also lists the query's **root tokens** from `subTokens` — the same source `parseTokens` validates against, so it cannot disagree with what `--column`/`--order`/`--filter` accept — and degrades to the members list with a note when that call is unavailable, keeping the command anonymous-capable per AC-61.4 (#98). *(Amended: the original listed members only, which meant the command whose purpose is telling you what you can ask for offered names the server rejects.)*
