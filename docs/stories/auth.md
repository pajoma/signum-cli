...
- AC-04.6: A stored token is verified with `GET api/auth/currentUser` **at the point it is established or inspected** — `auth login` and `auth status` — not before every command. A malformed, tampered, or wrong-key token is never rejected on its own terms, so the check is what makes a bad paste fail at login. It surfaces **two ways depending on the target app**: a 200 with a `null` body where `AuthLogic.AnonymousUser` is configured, or a **403** where it is not — the target application does the latter (observed 2026-07-28, #83). Both must produce the same actionable message. *(Amended: verifying on every invocation would double every command's round trips to re-prove something that only changes when the credential does. A token that stops working mid-life surfaces as the 403 it causes, which AC-08.2 already discriminates and AC-12.11 already routes.)*
...
**Acceptance Criteria:**
- AC-12.1: `signum auth login` reads the token from a **hidden terminal prompt** or from **stdin**, never from a command-line argument (shell history, process list). A token passed as an argument is **refused with a warning that it is now compromised** — it used to be silently ignored, which left it in the user's history for no benefit at all. `--with-token` is **optional** while the handoff is the only mechanism this application supports; requiring a flag that selects the only option made the obvious command fail. It stays meaningful for when REQ-002/REQ-004 mechanisms exist.
...
- AC-12.3: The token is validated immediately via `GET api/auth/currentUser` and the resolved user is echoed, so a bad paste fails at login rather than mysteriously later. Both rejection shapes (403, or 200-with-null — AC-04.6) must report the **handoff instructions**, not a generic "re-authenticate" hint: the user needs to know how to obtain a correct token, which is the whole point of failing at login.
...
- AC-12.8: The token is treated as a bearer secret throughout — redacted per STORY-11, never echoed back after entry, **terminal echo suppressed while pasting on a TTY**. *(Reinstated. An earlier amendment removed this on the grounds that TTY paste was refused outright — circular reasoning, since that refusal was itself the defect: it made the CLI's own GETTING STARTED command fail every time. "Never an argument" does not require "never typed"; a hidden read keeps the token out of both shell history and the process list, which is what this AC protects. Input is fully silent rather than masked with asterisks, matching `sudo`/`ssh` and avoiding leaking the length.)*
