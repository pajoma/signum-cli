...
- AC-52.12: An **entity reference is always an identity**, judged by VALUE rather than by member name, and becomes a `ref_` handle under any pseudonymizing mode. Name-based rules cannot catch these: an entity-valued column is called `User`, `Customer` or `Owner` — none of which matches a heuristic — while its value denotes a person and renders as that person's label. Two individually-correct behaviours (label rendering, name-based classification) therefore combined to print real names under `heuristic`; the leak was only visible once both were in the same build. Columns whose values are an entity's label because `--resolve` rewrote them to `.ToString` are treated the same way, since the CLI knows it asked for them. *(Amended, #105: an entity's **display string is identity-bearing wherever it appears**, not only where `--resolve` put it. `ToString` as the last segment of a query token and the wire field `toStr` are recognised structurally — both are exact names from the framework (`EntityToStringToken.Key`) and the wire, not guesses — and are reported as `entity-label` rather than `identity` so the introspection view says which rule fired. This was a leak of the **default**: a display string arrives as a plain string, so the value rule cannot see it, and it is called `ToString`, which matches no heuristic, so both halves of the classifier passed a person's name through with no flag involved. `--resolve` was already safe, which is what hid it — the covered path was the one everybody thought of, while `--column Entity.ToString` and `get`'s unconditional `toStr` were not, and `explain --privacy` reported the source member as protected on every path. `get ref_…` was the worst case: the command documented as the safe way to act on a concealed record disclosed the very thing the handle conceals.)*
...
**Acceptance Criteria:**
- AC-53.1: In pseudonymizing mode, `Lite` keys are emitted as opaque local handles instead of `TypeName;id`. *(Amended twice. **Width:** the handle carries **48 bits** — `ref_7f3a1c2b4d5e`, not the illustrative `ref:7f3a`. 16 bits collide at a few hundred entries by the birthday bound, and a handle collision means two people sharing one identity — precisely the silent mismatch AC-53.6 forbids. The store also detects a collision on write rather than trusting the arithmetic. **Separator:** `ref:` became **`ref_`** once AC-53.10 made handles appear in file and folder names. `:` is reserved in Windows filenames, so the original form was unusable in the one place `unmask --in` most needs it — an agent could not name a report after the handle it was handed, and had to either leak an identity into the path or invent its own mangling. `_` is legal on every platform, safe unquoted in a shell, and unlike `.` does not disturb extension parsing. The handle is still deliberately not `Type;id`-shaped. `ref:` remains **accepted** and is no longer minted: every handle store and document written before the change would otherwise be orphaned, which would self-inflict the failure AC-53.5 exists to prevent. Lookup tries the token as written and then the same digest under the other prefix, so an old store serves a new document and a new store serves an old one.)*
- AC-53.2: Any argument accepting a `Lite` also accepts `ref_…` (and the legacy `ref:…`), resolving it locally **before** the request is built.
...
- AC-53.10: `unmask` resolves handles **inside files**, not only as arguments (#102). The primitive was the right one at the wrong granularity: the workflow the handles exist for ends with an agent having written a document keyed by surrogates that a human now needs to read, and doing that by hand means grepping handles out, shelling out, and substituting back — about 150 lines, reinvented per consumer, with a silent corruption in the substitution step whenever the handle list is not sorted longest-first. Substitution is therefore **one left-to-right pass** over the text, which makes length collisions structurally impossible rather than avoided by care, and is idempotent because a replacement is a `Type;id` that cannot match the handle pattern. A handle-shaped token that is absent from the store is left byte-identical and **named** in the report: silently passing over it is indistinguishable from "there were none", which is how someone concludes a document was clean when the store was merely empty. Handles in **file and folder names** are resolved as well as in contents: `docs/ref_cccccccccccc/report-ref_aaaaaaaaaaaa.md` publishes two identities in the path alone, whatever the bytes inside say. This is what forced the separator change recorded in AC-53.1 — with the original `ref:` the case was unreachable on Windows, so the feature would have been a POSIX-only carve-out rather than a property of the tool. The default writes the sibling copy at the **resolved** path, so a copy carries no handle in its own name or in any folder above it; `--in-place` renames the originals, **deepest first** — renaming a parent before its children invalidates the child paths and the child renames then silently do nothing — and never over an existing name, because a rename that clobbers destroys data derived from a handle. A skipped binary is still reported when its NAME carries a handle, since renaming needs no reading. Reported paths are reconciled against where files **actually** ended up: leaf-by-leaf renaming is correct on disk but leaves intermediate paths recorded, and a summary naming a location that no longer exists is worse than no summary. Files are read and written as **UTF-8 explicitly** — the scripted version decoded CLI output using the platform codepage and produced mojibake in every German name. The input is **not** mutated by default (a `.local.` sibling is written, an infix a single `*.local.*` gitignore line covers), because the input is usually generated, tracked, and deliberately identity-free. This mode carries **more** risk than the original, not less: a printed name is transient and a written one is durable, so the gate is tested for this mode specifically rather than assumed to be inherited, and the CLI warns when an output file sits in a git work tree and is not ignored — the actual footgun being a commit full of real names in a repository that deliberately held none. `git check-ignore` is asked rather than gitignore semantics reimplemented, because wrongly reporting a file as ignored is worse than not checking.
- AC-53.11: The handle store keeps the entity's **display string** beside its identity, captured at mint time, so `unmask` produces a document a human can read rather than a page of `Project;20` (#106). **Where the label comes from:** a `Lite` on the wire carries no `toStr` — verified against `LiteJsonConverter.cs:26-64`, which writes `EntityType`, `id` and optionally `ModelType`, `partitionId`, `model` and `entity`. The display string travels in **`model`**, as a bare string when the model type is the default; a custom `ModelEntity` arrives as an object and is deliberately NOT treated as a label, because flattening structured data to a name would be a guess and a wrong label is worse than none. Capture happens inside `surrogate`, the last point at which the CLI holds both the identity and the label — the agent still receives only the handle. **`--labels`** selects what is substituted: `identity` (the pre-#106 Lite key), `stored` (default, offline), or `fetch`, which asks the server for the labels it lacks, one round trip per entity type, and caches them. `fetch` is the only part of `unmask` that touches the network, and it is opt-in: what must never leave the machine is the surrogate↔identity **mapping**, and a request saying `Id IsIn (20,21)` for one type carries no surrogate and tells the server nothing it does not already know about its own rows — asserted by a test that the handle never appears on the wire. A handle that resolves but has no label falls back to the identity and is **named** in the report, since a `Project;20` where a name was asked for is otherwise indistinguishable from a label that happens to look like a Lite key. Two hazards are handled rather than hoped about: a substituted value is made safe **for the format of the file being written**, decided per file from its extension — markdown gets its `|` escaped because one raw pipe silently shifts every cell after it, and everything else is substituted literally. *(Amended after review: escaping was applied to every text file, so a label containing `|` was written into JSON as an invalid `\|` escape and made the document unparseable. Format policy now has a home of its own in `core/textfiles.ts` — the review's point was that the bug entered because there was no boundary where such a rule belongs. CSV is deliberately left literal: there is no correct in-place escape for a delimiter inside a field without re-quoting it, which means parsing the CSV, and a guess would be worse than the stated limitation.)*; and a label used in a file or folder name is **sanitised and truncated** — Windows-reserved characters and both separators replaced, trailing dot or space removed (Windows would strip them silently and merge two distinct names), capped at 80 characters, **Windows device names** (`CON`, `NUL.txt`, `COM1`) neutralised at any casing, and the **composed** segment validated in bytes rather than only the label, since the caller wraps it and a legal original can compose into an oversized name — with the existing never-overwrite rule still deciding collisions, and an illegal name reported *distinctly* from a taken one so the two are not conflated. **The tradeoff, stated because it is one:** storing labels turns `handles.json` from a store of identities into a store of **names** — real personal data at rest, `0600` beside the credential, forgotten by `--clear`. `privacy.json` `{"storeLabels": false}` keeps identities only, and applies to the **whole store** on the next write rather than only to incoming handles — otherwise every name captured before the setting stayed on disk while the user reasonably believed the store was clean. `unmask --forget-labels` is the immediate migration, in which case `--labels fetch` still resolves them per run without writing them down, and `signum help pseudonymization` says plainly what the file holds. The store reads **both** shapes, so the thousands of bare-string entries written before this keep resolving.
...
- AC-53.7: `m2` — Mutations performed via a `ref_` handle are recorded in the local audit log (AC-46.6) with the **real** target, so an operator can reconstruct what actually happened. **Not reachable yet:** this milestone has no write commands, so there is no mutation to log. The handle-resolution half it depends on is in place.
