...
| REQ-008 | v1 | **Browser token handoff.** `auth login --with-token` accepts a Signum bearer token obtained from an existing browser session (`sessionStorage.authToken`, `AuthClient.tsx:189,198`), read from **stdin only**. **The primary path for the target application**, because it needs no change to the Signum configuration and none to the Entra tenant — the browser satisfies SSO/MFA/Conditional Access. Auto-upgrades to a durable API key via `GET api/restApiKey/current` when one exists ([ADR 0004](decisions/0004-entra-primary-identity-provider.md) Decision 3, [STORY-12](stories/auth.md)). |
