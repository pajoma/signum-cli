...
| REQ-058 | m2 | **Local re-identification mapping.** Emit opaque `ref:…` handles in place of `Lite` keys so an agent can act on a record it cannot identify; accept `ref:…` wherever a `Lite` is taken and resolve locally before the request. Mapping stored `0600` and **never** emitted in stdout, `--json`, MCP results, traces, or telemetry. Human-only `unmask` command; mutations via a handle are audit-logged against the real target. *(The command is `unmask` — short and guessable, since CLI verbs are typed daily. The concept keeps its precise name: this is **re-identification**, not un-redaction, because a pseudonym was substituted rather than a value hidden.)* See [STORY-53](stories/privacy.md). |
