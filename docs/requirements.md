...
| REQ-004 | spike | **Browser-based login.** `gh auth login --web` equivalent. **Feasibility unknown.** Analysis found no CLI-oriented browser flow: `AuthTokensServer` is username/password only. Likely needs the app's OpenID/AzureAD module plus a loopback redirect, or a server-side "authorize this CLI" page that does not exist today. Spike must answer: is this possible without framework changes? If not, propose the upstream change. |
| REQ-005 | spike | **External IdP authentication.** Target apps may use `Signum.Authorization.{WindowsAD,AzureAD,OpenID}`. Determine what each supports headlessly (device code? client credentials? PAT?) and which are reachable without a browser. Depends on REQ-004's findings. |
...
| REQ-002 | [#2](https://github.com/pajoma/signum-cli/issues/2) | `v1` | API-key authentication |
| REQ-003 | [#3](https://github.com/pajoma/signum-cli/issues/3) | `v1` | Username/password → bearer |
