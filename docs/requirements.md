...
| REQ-041 | m2 | **operationKey resolution.** Keys are `ContainerClassName.FieldName`, **not** namespace-qualified (`Signum/Basics/Symbol.cs:22`). **Operations are first-class commands** — the key *is* the command (`signum Order.Ship …`), dispatched by the rule that a first argument containing a `.` is an operation key and no built-in ever contains one ([CLI surface](design/cli-surface.md) §2.1). Resolve friendly input against cached metadata; disambiguate rather than guess. |
